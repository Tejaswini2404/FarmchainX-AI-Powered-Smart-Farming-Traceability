...
public class AdminHistoryService {
    ...
    public AdminUserHistoryResponseDto getUserHistory(Long userId) {
        ...
        case "farmer" -> {
            ...
            var products = productRepository.findByFarmerId(userId);
            data.put("productsAdded", products);

            data.put("ordersReceived", orderRepository.findByFarmerId(userId));
        ...
        case "customer" -> {
            data.put("ordersPlaced", orderRepository.findByCustomerId(userId));
        }
